    ...
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary/20 selection:text-primary">
                ...
                <>
                  ...
                  {userPlan === "premium" && (
                    <Link
                      ...
                      className={`flex items-center gap-2 text-sm ${
                        location.pathname === "/provider/organize-profile"
                          ? "text-green-600"
                          : "text-gray-700 hover:text-green-600"
                      }`}
                    >
              ...
              {isLoggedIn && userRole === "business" && (
                <>
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-2 text-sm ${
                      location.pathname === "/business/add-provider"
                        ? "text-purple-600"
                        : "text-gray-700 hover:text-purple-600"
                    }`}
                  >
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        {unreadCount > 0 && (
                          <span
                            className={`absolute top-0 right-0 w-4 h-4 text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white ${userRole === "business" ? "bg-pink-600" : "bg-green-600"}`}
                          >
                            {unreadCount}
                          </span>
                        )}
                          ...
                          <div className="max-h-96 overflow-y-auto">
                            {notifications.length > 0 ? (
                              notifications.map((n) => (
                                <div
                                  key={n.id}
                                  className={`p-4 border-b hover:bg-gray-50 cursor-pointer transition-colors ${!n.read ? "bg-red-50/30" : ""}`}
                                >
                    ...
                    <button
                      onClick={() => {
                        if (
                          confirm(
                            "Deseja zerar toda a simulação? Isso apagará todas as contas e orçamentos criados.",
                          )
                        ) {
                          resetAllData();
                          resetSimulation();
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs uppercase shadow-sm transition-all ${
                          userRole === "business"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-4">
              ...
              <p className="text-sm text-slate-500 max-w-sm leading-relaxed">
                A plataforma mais confiável para conectar clientes e
                profissionais qualificados. Serviços rápidos, seguros e práticos
                ao seu alcance.
              </p>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-50 flex justify-around items-center py-3 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] md:hidden">
        <Link href="/" className="flex flex-col items-center gap-1 group">
          ...
          <span className="text-[10px] font-bold uppercase tracking-tight text-gray-500 group-hover:text-green-600">
            Prestadores
          </span>
        </Link>
        <Link
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-tight text-gray-500 group-hover:text-pink-600">
            Empresarial
          </span>
        </Link>
      </div>
