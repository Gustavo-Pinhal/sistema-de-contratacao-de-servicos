...
export function ManageService() {
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4 font-black uppercase">
            Serviço não encontrado
          </h1>
          <Link
            href="/dashboard"
            className={`text-${themeColor}-600 hover:underline font-bold`}
          >
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "quote":
        return "bg-yellow-100 text-yellow-800 border-yellow-200 shadow-sm shadow-yellow-50";
      case "active":
        return `bg-${themeColor}-100 text-${themeColor}-800 border-${themeColor}-200 shadow-sm shadow-${themeColor}-50`;
      case "completed":
        return "bg-green-100 text-green-800 border-green-200 shadow-sm shadow-green-50";
      case "cancelled":
        return "bg-red-100 text-red-800 border-red-200 shadow-sm shadow-red-50";
      default:
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-gray-100 p-8 mb-8 border border-gray-50">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            ...
            <div
              className={`inline-flex items-center px-6 py-2.5 rounded-xl border-2 font-black text-[10px] uppercase tracking-widest ${getStatusColor(service.status)}`}
            >
            ...
            <div className="bg-white rounded-2xl shadow-xl shadow-gray-100 flex flex-col h-[650px] border border-gray-50 overflow-hidden">
              <div className="p-6 border-b flex items-center justify-between bg-white/50 backdrop-blur-md sticky top-0 z-10">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-3 h-3 bg-${themeColor}-500 rounded-full animate-pulse`}
                  ></div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/30">
                {service.messages.map((msg) => {
                  const isMe =
                    (userRole === "client" && msg.sender === "client") ||
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-xl shadow-gray-100 p-8 border border-gray-50">
              ...
              {service.proposedValue && !showValueInput && (
                <div
                  className={`mb-6 p-6 bg-${themeColor}-50 rounded-2xl border-2 border-${themeColor}-100 shadow-md shadow-${themeColor}-50/50`}
                >
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">
                    Valor do Orçamento
                  </p>
                  <div className="flex items-center justify-between">
                    <p
                      className={`text-3xl font-black text-${themeColor}-600 tracking-tighter`}
                    >
                ...
                service.proposedValue && (
                  <div className="mb-6 p-6 bg-yellow-50 border-2 border-yellow-100 rounded-2xl">
                    <p className="text-sm text-yellow-800 font-bold mb-4 leading-relaxed">
                      O prestador enviou uma proposta de{" "}
              ...
              <div className="space-y-3">
                ...
                {service.visitScheduled && (
                  <div
                    className={`p-6 border-2 rounded-2xl ${
                      service.visitScheduled.status === "confirmed"
                        ? "bg-green-50 border-green-100"
                        : service.visitScheduled.status === "cancelled"
                  ...
                  >
                    ...
                    >
                      {new Date(service.visitScheduled.date).toLocaleDateString(
                        "pt-BR",
                      )}{" "}
                      às {service.visitScheduled.time}
                    </p>
                    {userRole === "provider" &&
                      service.status !== "completed" &&
                      service.visitScheduled.status !== "cancelled" && (
                ...
                {service.status === "completed" && (
                  <div className="p-6 bg-green-50 border-2 border-green-100 rounded-2xl text-center">
                    ...
                    <h4 className="text-green-900 font-black uppercase text-sm mb-1">
                      Serviço Concluído
                    </h4>
                ...
                {service.status === "cancelled" && (
                  <div className="p-6 bg-red-50 border-2 border-red-100 rounded-2xl text-center">
                    <h4 className="text-red-900 font-black uppercase text-sm mb-1">
                      Serviço Cancelado
                    </h4>
            ...
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-8 text-white shadow-2xl relative overflow-hidden">
              <div className="relative z-10">
                <h4 className="text-lg font-black uppercase tracking-tight mb-4">
                  Dicas de Segurança
                </h4>
                <ul className="space-y-4 text-xs font-medium text-gray-300 leading-relaxed list-disc pl-4">
                  <li>
                    Recomendamos não realizar pagamentos antecipados. O
                    pagamento deve ser feito diretamente ao prestador após a
                    conclusão do serviço.
                  </li>
                  <li>
                    Sempre verifique o perfil e as avaliações do prestador.
                  </li>
