...
export function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex items-center gap-6">
              {isLoggedIn && userRole === 'client' && (
                <Link href="/search" className={`flex items-center gap-2 text-sm ${pathname === '/search' ? 'text-blue-600' : 'text-gray-700'}`}>
              ...
              <div className="flex items-center gap-4 border-l pl-6 border-slate-200">
                {!isLoggedIn ? (
                  <>
                    <button onClick={() => setLoginDialogOpen(true)} className="text-sm font-semibold text-slate-700">Entrar</button>
                    <button onClick={() => setRegisterDialogOpen(true)} className="bg-indigo-600 text-white px-5 py-2 rounded-full text-sm font-semibold">Cadastre-se</button>
                  </>
                ) : (
                  <div className="flex items-center gap-5">
                     {/* Botoes de Notificação, Zerar e Logout permanecem iguais */}
                     <button onClick={() => logout()} className="text-gray-400 hover:text-red-500"><LogOut className="w-5 h-5" /></button>
                  </div>
                )}
              </div>
            </nav>
      ...
      {/* Dialogs */}
      <RoleSelectionDialog isOpen={loginDialogOpen} onClose={() => setLoginDialogOpen(false)} type="login" />
      <RoleSelectionDialog isOpen={registerDialogOpen} onClose={() => setRegisterDialogOpen(false)} type="register" />
    </div>
  );
}