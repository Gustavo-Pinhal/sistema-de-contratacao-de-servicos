    ...
    <div className="grid md:grid-cols-2 gap-4">
      {plans.map((plan) => (
        <div
          ...
          className={`relative flex flex-col p-6 rounded-2xl border-2 transition-all cursor-pointer ${
            selectedPlanId === plan.id
              ? type === "business"
                ? "border-pink-600 bg-pink-50/50 ring-2 ring-pink-600 ring-offset-2"
        ...
        >
          {plan.isPopular && (
            <div
              className={`absolute -top-3 left-1/2 -translate-x-1/2 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-sm ${
                type === "business"
          ...
          <ul className="space-y-3 mb-6 flex-1">
            {plan.features.map((feature, idx) => (
              ...
              >
                <Check
                  className={`w-4 h-4 mt-0.5 shrink-0 ${selectedPlanId === plan.id ? (type === "business" ? "text-pink-600" : "text-green-600") : "text-gray-400"}`}
                />
                {feature}
              </li>
            ))}
